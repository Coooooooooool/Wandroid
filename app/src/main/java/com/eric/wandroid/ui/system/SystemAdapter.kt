...
class SystemAdapter(
    ...
    private val items = mutableListOf<SystemListItem>()
    private var collectingArticleIds: Set<Int> = emptySet()

    fun submitState(state: SystemUiState) {
        items.clear()
        collectingArticleIds = state.collectingArticleIds
        when (state.mode) {
            SystemContentMode.System -> {
                items += state.roots.map { SystemListItem.ChapterGroup(it, state.selectedCategoryId) }

                if (state.selectedCategoryName.isNotBlank() || state.articles.isNotEmpty()) {
                    items += SystemListItem.ArticleSectionHeader(
                ...
                if (state.articles.isNotEmpty()) {
                    items += state.articles.map { SystemListItem.ArticleRow(it) }
                    items += SystemListItem.LoadMoreFooter(
            ...
            SystemContentMode.Navigation -> {
                if (state.navigationGroups.isNotEmpty()) {
                    items += SystemListItem.NavigationSectionHeader(state.navigationGroups.size)
                    items += state.navigationGroups.map { SystemListItem.NavigationGroupRow(it) }
                }
            }
        }
    ...
    override fun getItemViewType(position: Int): Int = when (items[position]) {
        is SystemListItem.ChapterGroup -> VIEW_TYPE_CHAPTER_GROUP
        is SystemListItem.NavigationSectionHeader -> VIEW_TYPE_NAVIGATION_HEADER
        is SystemListItem.NavigationGroupRow -> VIEW_TYPE_NAVIGATION_GROUP
        is SystemListItem.ArticleSectionHeader -> VIEW_TYPE_ARTICLE_HEADER
        is SystemListItem.ArticleRow -> VIEW_TYPE_ARTICLE
        is SystemListItem.LoadMoreFooter -> VIEW_TYPE_LOAD_MORE
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        return when (viewType) {
            VIEW_TYPE_CHAPTER_GROUP -> ChapterGroupViewHolder(
                inflater.inflate(R.layout.item_system_chapter_group, parent, false),
                onCategoryClick
            )
            VIEW_TYPE_NAVIGATION_HEADER -> NavigationHeaderViewHolder(
                inflater.inflate(R.layout.item_system_navigation_header, parent, false)
            )
            VIEW_TYPE_NAVIGATION_GROUP -> NavigationGroupViewHolder(
                inflater.inflate(R.layout.item_system_navigation_group, parent, false)
            )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            is SystemListItem.ChapterGroup -> (holder as ChapterGroupViewHolder).bind(item.root, item.selectedCategoryId)
            is SystemListItem.NavigationSectionHeader -> (holder as NavigationHeaderViewHolder).bind(item.count)
            is SystemListItem.NavigationGroupRow -> (holder as NavigationGroupViewHolder).bind(item.group)
            is SystemListItem.ArticleSectionHeader -> (holder as ArticleHeaderViewHolder).bind(item.categoryName, item.count)
            is SystemListItem.ArticleRow -> (holder as ArticleViewHolder).bind(
                item.article,
                collectingArticleIds.contains(item.article.id)
            )
            is SystemListItem.LoadMoreFooter -> (holder as LoadMoreViewHolder).bind(item)
        }
    }

    private class ChapterGroupViewHolder(
        itemView: View,
        private val onCategoryClick: (SystemChildCategory) -> Unit
    ) : RecyclerView.ViewHolder(itemView) {
        private val titleView: TextView = itemView.findViewById(R.id.chapterGroupTitle)
        private val chipGroup: ChipGroup = itemView.findViewById(R.id.chapterChipGroup)

        fun bind(root: SystemRootCategory, selectedCategoryId: Int?) {
    ...
    private class NavigationHeaderViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val titleView: TextView = itemView.findViewById(R.id.navigationTitle)
        private val countView: TextView = itemView.findViewById(R.id.navigationCount)

        fun bind(count: Int) {
            titleView.setText(R.string.system_navigation_title)
            countView.text = itemView.context.getString(R.string.system_navigation_count, count)
        }
    }

    private class NavigationGroupViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val titleView: TextView = itemView.findViewById(R.id.navigationGroupTitle)
        private val chipGroup: ChipGroup = itemView.findViewById(R.id.navigationChipGroup)

        fun bind(group: NavigationGroup) {
    ...
    private class ArticleHeaderViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(categoryName: String, count: Int) {
            titleView.text = if (categoryName.isBlank()) {
                itemView.context.getString(R.string.system_articles_title)
            } else {
                categoryName
            }
    ...
    private class LoadMoreViewHolder(
        ...
        fun bind(item: SystemListItem.LoadMoreFooter) {
    ...
    companion object {
        private const val VIEW_TYPE_CHAPTER_GROUP = 1
        private const val VIEW_TYPE_NAVIGATION_HEADER = 2
        private const val VIEW_TYPE_NAVIGATION_GROUP = 3
