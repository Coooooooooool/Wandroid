...
class SystemDetailViewModel(
    ...
    private val initialRootName: String,
    private val repository: SystemRepository,
    private val accountRepository: AccountRepository
) : ViewModel() {
    private val _uiState = MutableStateFlow(SystemDetailUiState(rootName = initialRootName))
    ...
    fun selectCategory(category: SystemChildCategory) {
        ...
        if (isBusy() || state.selectedCategoryId == category.id) return
            ...
            when (val result = repository.loadCategoryArticles(category, page = 0)) {
                ...
                is AppResult.Error -> {
                    _uiState.update {
                        it.copy(
                            isCategoryLoading = false,
                            blockingErrorMessage = if (it.children.isEmpty()) result.message else null
                        )
                    }
    ...
    fun loadMore() {
        val state = _uiState.value
        val selectedCategory = state.children.firstOrNull { it.id == state.selectedCategoryId } ?: return
    ...
    private fun loadDetail(isRefresh: Boolean) {
        ...
        viewModelScope.launch {
            when (val result = repository.loadRootDetail(rootId)) {
                is AppResult.Success -> {
                    val data = result.data
                    _uiState.update {
                        it.copy(
...
class SystemDetailViewModelFactory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(SystemDetailViewModel::class.java)) {
            ...
            return SystemDetailViewModel(rootId, rootName, repository, accountRepository) as T
        }
